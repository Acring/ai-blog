---
title: 整理AI 开发中的核心概念：MCP、Agent、Subagent 与 Skills 等等
date: 2026-03-02
category: tech
...
到了这一步，我们发现，AI 已经可以完成一些简单的任务了，但是这些任务往往是简单的，比如**回答问题**，或者**生成一些简单的文本**。但是一个问题就是，大模型的训练数据是老旧的，他并不知道现在的世界是怎么样的，所以在回答当前时间的信息的时候，往往会进行胡编乱造。**我们希望 AI 能够搜索互联网，获取最新的信息。最简单的版本，就是让 AI 告诉我们要搜索什么，然后我们自己去搜索，再返回给 AI**。非常麻烦。


这很显然是一个非常低效的方式，更方便的方法，我们编写一个函数，**函数的能力可以是搜索互联网**，然后让 AI **输出这个这个函数的名称，调用参数（要搜索的内容）**，然后我们**获取 AI 的返回结果，解析成函数调用，在程序内调用函数，再将函数调用结果返回给 AI**。这就是所谓 **function call** 的概念。

![./images/generated-1773888462573-1.jpg](./images/generated-1773888462573-1.jpg)

最开始 openai 的 api 是有两个参数 **functions(模型可能生成 JSON 输入的函数列表。) 和 function_call(控制模型调用哪个（如果有的话）函数。)**，这个命名主要是延续了程序员的思想，现在已经被 **tools** 和 **tool_choice** 替代了，从命名上更符合大众的认知。也更加通用。

所以虽然现在我们还说 function call，但是实际上我们说的是 tools 和 tool_choice。

![./images/1773890631188.png](./images/1773890631188.png)
...
通过 tools 参数，我们告诉大模型，你可以使用 web_search 工具去进行搜索。这个 web_search 工具是 anthropic 提供的，其他厂商的大模型，也会提供自己的工具。可能会相应的收取一定的工具调用费用。返回结果如下 
...
可以看到返回结果中，有一个 `server_tool_use` 字段，这个字段就是**大模型调用工具的记录**。当大模型认为需要调用 `web_search` 进行搜索的时候，
...
当然这里的搜索工具是由 `anthropic` 提供的，属于**服务端工具**，他的问题是并不是很通用，所以除此之外还可以自己定义**客户端工具**。
...
可以看到，这里返回了**大模型返回了工具使用的记录**，而这时候我们拿到 `tool_calls` 字段，就可以知道大模型需要调用 `get_weather` 工具，并且传入了 `location` 参数。
...
但是**总体结构又差不多，都是 name ,description, 传入参数(input_schema, parameters)。大同小异。**
...
还需要注意一点的是，能返回这样的格式，实际上是大模型训练的结果，AI 厂商会针对性的训练大模型，让他能够更好的返回这样的 json 格式。所以才能支持 `tools` （`function call`）。
在最初的时候，很多大模型并不支持 `function call`，所以只能返回 `text` 类型的内容。那时候，如果需要让大模型调用工具，人们会在提示词中手动要求生成 `xml`，或者 `json` 的格式，然后用代码
解析 `text` 内容，并调用工具。也算是一种 hack 的方式。

总体来说，`function call` 是 **AI 技术发展过程中的一个重要里程碑，它让 AI 能够调用工具**，从而拥有了更多的能力。是我们后面很多概念的基础。然而由于大部分人根本没有调用过大模型的
api，所以对于这个概念并不熟悉，甚至很多都不知道有这个概念。
...
到了这一步，我们其实已经实现了一个 Agent，**是的，并没有那么复杂，有个了「对话+规划+工具调用+总结结果」，这就已经是一个 Agent 了**。至于后面更加强大的 Agent ，他本质上就是提供更多的工具，更合理的对话上下文管理，更强大的规划能力，更强大的总结能力。其本质都是对于这些能力的增强。

![./images/generated-1773892740461-1.jpg](./images/generated-1773892740461-1.jpg)

所以从概念来说，我们可以评价为「夯」，因为一个 Agent 就是后面各种强大 AI 工具能力的基础架构。但是人们在神话这个概念，企图把它定义为 AGI，这是不现实的。

## Workflow

虽然 Agent 在概念上非常火，但是我们不能忽略了 Workflow 的概念，他和 Agent 最大的差别在于工具的调用是工程系统固定的还是由 AI 自行决策的。有一些功能，他的流程是固定的，那他其实并不需要 AI 来判断是否需要调用工具，调用什么工具，他的结构类似于

```
输入 → Step1 → Step2 → Step3 → 输出
```

![./images/generated-1773892619939-1.jpg](./images/generated-1773892619939-1.jpg)

这种情况下，我们应该使用 **workflow** 而不是 **agent**，所以虽然从火爆程度来说，**workflow** 远不如 **agent**，但是从实际应用角度来说，**workflow** 同样是非常实用的。我们需要去考虑到底是否真的需要 **Agent** 来完成这个任务。而不是无脑堆砌 Agent。

所以我们同样给到「夯」。

还需要注意的是，在一个成熟的系统中，往往不会二选一，而是会采用 **Workflow + Agent 混合架构**，
`Agent` 可以调用某一个 `Workflow` 来完成任务，一个 `Workflow` 也可以在某一步调用 `Agent` 进行可约束范围的决策。


## Rule 

现在我们有一个能够帮助我们实现各种任务的 `Agent`，但是这个 `Agent` 脑子还有点笨，他不知道我们在不同任务时需要遵循什么样的规范，比如说，我编写一个 react 项目，**我需要遵循什么样的代码结构，我需要遵循什么样的命名规范，我需要遵循什么样的代码风格，我需要遵循什么样的代码规范，我需要遵循什么样的代码最佳实践，我不应该写什么代码**。
...
规则的概念，让 `Agent` 能够遵循规范，从而实现更准确的任务执行。**但是规则慢慢被滥用，成为了一种预制的提示词**，比如说我要让 `Agent`，执行一段工作流，我就会把工作流写成一个规则，然后在对话开始的时候，将 `rule` 引用过来，让 `Agent` 去遵循。像我之前就经常这么用，这么用当然可以，因为本质上， rule 就是提前插入一段提示词，至于这段提示词是规范，还是指令，对于底层来说，区别不大，但是从语义规范上来说，还是应该区分一下。

**规范是用来约束 Agent 行为的，而指令是用来描述 Agent 执行任务的流程和目标的。** 那 claude 就推出了 `commands` 的概念，`commands` 是用来描述 Agent 执行任务的预制提示词的，我们可以在对话开始的时候，将 `commands` 引用过来，让 `Agent` 去执行，而不需要每次都写一遍。

**不过这个 `commands` 的概念没多久， `skills` 的概念就出现了，并且 `skills` 的概念更加成熟，更加强大，更加灵活。**

`claude` 也顺势将 `command` 的概念提升为了 `skills` 的概念，所以我们基本没怎么听说过 `commands `的概念。

评价为 「NPC」，只是昙花一现一现的中间概念。
...
在之前我们提到了 `function call` 的概念，`function call` 是 AI 模型调用工具的协议，当我们的客户端接收到这个协议之后调用自己实现的工具函数，并返回结果给 AI 模型。但是这个工具必须是客户端实现的，但是如果我希望客户端能够调用外部的工具，能够随时插入新的工具，而不需要每次都修改客户端代码，这时候我们就需要 `MCP` 了。
...
是的，因为他们都是 anthropic 定义的，所以他们的结构是类似的。 **`MCP` 也不是为了取代 `function call`，而是 `MCP` 的工具就是为了能够单独定义，而不需要集成在客户端设计的，而且底层也依赖于 `function call` 的协议。这是很多人容易搞混的地方。**

`MCP` 会通过 `tools/list` 暴露自己提供的工具列表，客户端会根据这个列表，构造 `tools` 参数，发给大模型，来选择自己需要的工具，然后通过 `tools/call` 调用工具。获取返回结果。

这里的 `tools/call` 其实也是和 claude 的 `function call` 返回的 `tool_calls` 字段是类似的，都是工具调用的记录。**所以其实整个流程是 MCP 服务端到 MCP 客户端使用 MCP 规范，客户端调用 AI，遵循 tool call 规范，而这两个规范从参数上基本一致，配合着完成整个流程。**

**MCP 总的来说是非常好的概念，他让外部工具调用有一个一套统一的协议，让 Agent 客户端能够方便的调用外部工具。而不需要在客户端集中开发。**
...
并且由于开发的门槛不高，各种各样的 MCP 如雨后春笋般出现，这就导致了**大量的重复和低质量的轮子**，很多也缺乏维护，导致大家也懒得去花时间筛选，甄别哪些是好的，最终成了一个「开发者大于使用者」的尴尬局面。

于是很多人又从推崇 MCP 变成了把 MCP 贬的一无是处。实际上作为开发者，我们需要意识到 MCP 就是一个协议，一个标准，一个规范，他本身并没有好坏，好坏在于我们如何去使用他，如何去实现他。

总的来说，MCP 可以评价为**顶级**，他是一个值得我们关注和研究的概念。
...
1. 创建一个文件夹，根据用户的主题，创建一个文件夹，例如 `drafts/[主题]/`
2. 创建一个新草稿，根据模板 `templates/post.md` 创建一个新草稿
3. 将草稿保存到 `drafts/[主题]/[文章标题].md` 目录下
```
...
你只需要在 SKILL.md 文件中说明，**什么时候需要加载引参考文档，执行捆绑的代码。并且由于是文件夹，skills 天生就可以跟着项目仓库进行管理，可以被版本控制，可以被分享，可以被复用**。同时 skills 又可以作为一个单独的项目，进行独立的开发，测试，部署，管理。有点类似于 shadcn 的组件库的概念。

比如我可以把 skills 放在 github 仓库，然后用 

```bash 
 npx skills add git@gitlab.xsky.com:front-end/agent-skills.git
```

他会自动克隆仓库，然后识别其中的 skills ，然后选择合适的 skills 安装到本地。所谓安装，其实也就是复制文件到本地目录。

`skills` 是 `vercel` 团队推出的，[https://github.com/vercel-labs/skills](https://github.com/vercel-labs/skills) 仓库，
他们还维护一个 skills 市场 [https://skills.sh/](https://skills.sh/) ，你可以在这里找到很多优秀的 `skills` ，并且可以在这里分享你的 `skills` 。

**当然我觉得大部份 `skills` 还是需要自己来开发，这样才能最对应当前项目的需求，或者基于别人的 `skills` 进行改造，以满足当前项目的需求。**

在最开始的时候，不同的工具对应的 skills 目录都不同，比如 .claude/skills, .cursor/skills, .openai/skills 等等，**但是随着 skills 的普及，这些工具都逐渐支持了 .agents/skills 目录，这样就不用安装到不同的目录了，当然目前来说 `skills` 工具会推荐通过软连接的方式，将 .agents/skills 目录链接到其他的 .skills 目录，这样就可以统一管理了。**

**而跟着 skills 一起火起来的，是一个已经看起来过时的老东西: CLI 工具**。原因也很简单，大模型本身的训练中就带了大量的 shell 环境训练，所以在执行命令行上非常的得心应手，本身 shell 自带的命令行工具基本不需要额外的提示词，AI 就能非常自如的执行命令行。而新的工具，

在之前的时代，大家都习惯使用 GUI 工具，是因为 CLI 工具的参数太多，使用麻烦，编写麻烦成了大家使用的门槛，比如你需要通过 -h 去查看各种参数的用法， 然后根据帮助信息，构造 CLI 命令，执行后读取命令行结果，再返回给 AI 模型。**但是对于 AI 来说，却是非常方便的工具，只需要 -h 或者 --help 就能获取帮助信息，然后根据帮助信息，构造 CLI 命令，执行后读取命令行结果，再返回给 AI 模型。而正是由于这种方式，反而契合了 skills 的渐进式披露的特性，让 Token 的使用更少。**

这个过程的方便程度对比 MCP 有过之无不及，同时 CLI 不仅 Agent 能用，人也能用，调试也方便，安装也不需要依赖于外部市场，各种语言基本都有自己的包管理工具来安装 CLI。比如 `npm` , `python uv` , `brew` 等等。

**在这种情况下，Skills + cli 的组合迅速走火，相比 MCP 更火，并迅速成为了 AI 开发中的主流概念。在我看来，Skills 的概念其实出现的比预期的晚太多了，早在 cursor 的第一版 rule 规范中，就有 markdown 的 metadata 的 name, description，的概念**，结果一直到 anthropic 推出 skills 的概念，才真正让 skills 的概念火了起来。实在是很令人费解。因为相比复杂的 MCP 来说，skills 的概念其实更加简单，更加灵活。

不过就像是 angular 的出现比 react 跟早一样，一开始的技术架构往往更「重」，但随着在实践中的经验积累，人们发现，其实很多复杂的需求，其实是可以通过简单的架构来实现的，skills 就是这样一个例子。

**而从概念来说，skills 可以评价为「夯」，一个极低的门槛带来的极高的上限，尤其是搭配 cli 工具的情况下，让很多 AI 开发者在短短几分钟内，就能实现一个非常强大的技能。**
...
从上面的介绍也可以看出来，虽然主观上 skills 和 MCP 并不是替代关系，但是**客观上 skills 会在很多需求上成为替代 MCP 的选择，尤其是对于一些简单的需求，比如读取文件，执行命令行等等，使用 skills 更加方便快捷。在实际开发中，我们也会优先选用 skills 而不是 MCP。**，只有遇到 skills 解决不了的问题，才会考虑使用 MCP。

钉钉CEO陈航表示:“今天，我们把钉钉打碎，用AI重建，炼出‘悟空’。过去是人用钉钉来工作，未来是AI用钉钉来工作。和市面上所有的龙虾Agent不一样，悟空天然就长在企业组织中，可以在真实的企业环境中安全使用。”
具体来看，钉钉重写底层代码，进行全面CLl(命令行界面)化改造，让悟空Agent能够原生操作钉钉上千项能力，而非模拟人类点击图形界面，实现“沟通即执行”。悟空全面支持连接用户在企业中的钉钉账号、安全访问权限和应用系统。同时，阿里生态业务ToB能力将以skils形式逐步嵌入该平台，作为阿里AI能力在企业工作场景的统一出口。

从未来趋势来看， 工具 cli 化，技能化，将成为 AI 开发的主流趋势。举一个具体的例子，
[https://github.com/4ier/notion-cli](https://github.com/4ier/notion-cli) 是一个 notion cli 的工具，而他本身又在仓库中写了 skills ，我们只需要安装工具
...
那么这个 agent 就拥有了使用这个 CLI 工具进行工作的能力，非常的简单方便。

总结评价，必须给到 **夯**

### 如何实现

这里还有一个补充，skills 是如何实现的，从 claude code 的抓包来看，他其实是把 skills 作为了一个 tool call 的一部份，让大模型判断要不要使用 skill，如何使用。所以其实他也不是什么黑科技，本质就是 prompt 拼接和 tool call 规范。

![images/1773903315069.png](images/1773903315069.png)
...
![images/agent-team.png](images/agent-team.png)

评价为 **NPC** 到 **人上人** 之间，大家只需要了解，在需要的时候想起来即可。
...
![images/1773053012858.png](images/1773053012858.png)
